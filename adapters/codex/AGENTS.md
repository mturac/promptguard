...
Do not wait for the user to say "promptguard".
...
- For pasted prompt text, run it through stdin before writing:
  `printf '%s' '<prompt text>' | python3 ~/.codex/skills/promptguard/scripts/audit_prompt.py - --format markdown`
...
- If that path does not exist, try `python3 ~/.codex/skills/promptguard/scripts/audit_prompt.py <file> --format markdown`.
- If the user asks for "the report" or does not name a specific file, you MUST run a repo-level audit immediately:
  `python3 skills/promptguard/scripts/audit_repo.py . --format markdown`
- If repo-local `audit_repo.py` does not exist, run:
  `python3 ~/.codex/skills/promptguard/scripts/audit_repo.py . --format markdown`
- If no script is available, explicitly say the script is unavailable and perform a manual audit in the same report format.
- Summarize script output; do not paste excessive logs.

Prompt-like files include names containing `prompt`, `agent`, `router`, `evaluator`, `system`, `AGENTS.md`, or `CLAUDE.md`.
